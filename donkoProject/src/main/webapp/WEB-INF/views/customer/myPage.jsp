...
<body>
  ...
  <thead>
    ...
    <th><strong>配送ステータス</strong></th><br>
  </thead>
  <tbody>
    ...
    <% for (PurchaseBean purchaseBean : purchaseList) { %>
      <tr align="center" style="display: flex; justify-content: space-between; height: auto; padding-left:90px;">
          ...
          <!-- 合計金額 -->
          <td width="20" style="padding-left:100px";>¥ <%= String.format("%,d",purchaseBean.getTotalAmount()) %></td>
          <!-- 購入日 -->
          <td width="20" style="padding-left:100px";><%= purchaseBean.getPurchaseDate() %></td>
          <!-- 配送先 -->
          <td width="20" style="padding-left:100px";><%= purchaseBean.getShippingAddress() %></td>
          <!-- 配送ステータス -->
          <td width="20" style="padding-left:100px";><%= purchaseBean.getShippingStatus() %></td>
      </tr>
    <% } %>
    </tr>
  </tbody>
